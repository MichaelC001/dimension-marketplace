...
function PulseMark({ size, active, hovered, animate, store }) {
	...
	const working = useSyncExternalStore(observable?.subscribe ?? noopSubscribe, () => observable?.getSnapshot() ?? NO_SESSIONS, () => NO_SESSIONS).some((session) => session.status === "running");
